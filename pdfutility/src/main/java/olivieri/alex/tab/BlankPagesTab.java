...
public final class BlankPagesTab {
    private BlankPagesTab() {
    }
    ...
    public static JPanel create(App controller, JFrame parent) {
        ...
        JButton processButton = new JButton("Aggiungi pagine bianche");

        browseButton.addActionListener(
                event -> browseForPdfFile(parent, inputField, outputField, controller::buildDefaultBlankPagesName));
        processButton.addActionListener(
                event -> controller.startInsertBlankPages(parent, inputField.getText(), outputField.getText(),
                        processButton));
        ...
        JLabel descriptionLabel = new JLabel("Inserisce pagine bianche finali per raggiungere il numero di pagine desiderato.");
